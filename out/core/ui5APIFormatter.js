...
exports.formatJsDoc = exports.convertModuleNameToPath = exports.filterApiMembers = exports.getFormattedObjectApi = void 0;
const constants = require("./constants");
const ui5APIService = require("./ui5APIService");
function getFormattedObjectApi(ui5ObjectApi, cleanHtml = false, inheritedAsArray = false, flatStatic = false) {
    const api = {
        ...
        description: formatJsDoc(ui5ObjectApi.description, cleanHtml),
    ...
    if (ui5ObjectApi.methods) {
        api.hasMethods = true;
        let methods = ui5ObjectApi.methods.filter((element) => {
            return element.visibility === "public";
        });
        api.methods = JSON.parse(JSON.stringify(methods));
        if (api.methods) {
    ...
    if (ui5ObjectApi.events) {
        api.hasEvents = true;
        let events = ui5ObjectApi.events.filter((element) => {
            return element.visibility === "public";
        });
        api.events = JSON.parse(JSON.stringify(events));
        if (api.events) {
    ...
    switch (ui5ObjectApi.kind) {
        case "class":
            ...
            if (ui5ObjectApi.hasOwnProperty("constructor")) {
                ...
                let construct = JSON.parse(JSON.stringify(ui5ObjectApi.constructor));
                if (construct) {
                    api.constructor = {};
                    api.constructor.description = formatJsDoc(construct.description, cleanHtml);
                    if (construct.parameters) {
                        api.hasConstructorParams = true;
                        api.constructorParams = JSON.parse(JSON.stringify(construct.parameters));
                        if (api.constructorParams) {
                            api.constructorParams.forEach((param) => {
                                ...
                                if (param.parameterProperties) {
                                    ...
                                    const parameterProperties = JSON.parse(JSON.stringify(param.parameterProperties));
                                    if (parameterProperties) {
                                        const properties = [];
                                        for (const prop of parameterProperties) {
                                            if (prop.description) {
                                                prop.description = formatJsDoc(prop.description, cleanHtml);
                                            }
                                            prop.objectName = ui5ObjectApi.name;
    ...
    if (ui5ObjectApi["ui5-metadata"] && ui5ObjectApi["ui5-metadata"].aggregations) {
        ...
        const aggregations = ui5ObjectApi["ui5-metadata"].aggregations.filter((element) => {
            return element.visibility === "public";
        });
        api.aggregations = JSON.parse(JSON.stringify(aggregations));
        if (api.aggregations) {
...
//# sourceMappingURL=ui5APIFormatter.js.map